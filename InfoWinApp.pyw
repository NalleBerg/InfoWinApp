...
def main():    
	...
	# Adding title
	Window.title("InfoWinApp V " + AppVersion)  # Initial titlw
	# with
	# Function to toggle "always on top"
	def toggle_topmost():
		if Window.attributes("-topmost"):
			Window.attributes("-topmost", False)
			Window.title("InfoWinApp V " + AppVersion)  # Reset title
			toggle_var.set(0)  # Uncheck the menu item
		else:
			Window.attributes("-topmost", True)
			Window.title("InfoWinApp V " + AppVersion +" - " + AlwaysOnTopText)  # Append " - Always on Top"
	...
	tabControl.add(tab10, text = Tab10Text)
	...
	if OnInternet:
 		# External IP
		try:
			external_ip_v4 = urllib.request.urlopen('https://ipv4.ident.me', timeout=1).read().decode('utf8')
		except urllib.error.URLError:
			external_ip_v4 = ServiceNotAvailableText

		try:
			external_ip_v6 = urllib.request.urlopen('https://ipv6.ident.me', timeout=1).read().decode('utf8')
		except urllib.error.URLError:
			external_ip_v6 = ServiceNotAvailableText
...
# Motherboard
	def get_motherboard_info():
		try:
			output = subprocess.check_output("wmic baseboard get product,Manufacturer,version,serialnumber", shell=True)
            # Split the output into lines and skip the header
			lines = output.decode().strip().split('\n')[1:]

            # Extract values from the first line
			product = lines[0].split('  ')[1]
			manufacturer = lines[0].split('  ')[0]
			version = lines[0].split('  ')[2]
			serial_number = lines[0].split('  ')[3]
